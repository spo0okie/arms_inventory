		...
		<?php if ($provider && count($map->nodes)) { ?>
			...
			<div class="col-auto form-check">
				...
				<label class="form-check-label" for="network-map-sweep"
					qtip_ttip="Перед опросом прогреть таблицы MAC: сервис пропингует сети площадки
						(записанные в инвентаризации: сеть → VLAN → L2-домен → помещение),
						ответы живых хостов пройдут через коммутаторы, и FDB станут плотными -
