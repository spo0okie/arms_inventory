	...
	'schedule'=>[
		'value'=>function($data) use ($renderer){
			if (is_object($data->acl) && is_object($data->acl->schedule))
				return $data->acl->schedule->renderItem($renderer,['static_view'=>false,'modal'=>true]);
	...
	'resource'=>[
		'value'=>function($data) use ($renderer,$glue){
			if (is_object($data->acl))
				return ModelWidget::widget(['model'=>$data->acl,'options'=>['static_view'=>false,'modal'=>true]]);
			return '';
		}
	],
	'resource_nodes'=>[
