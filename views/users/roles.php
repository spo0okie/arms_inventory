...
$roles=\Yii::$app->params['useRBAC']?Yii::$app->authManager->getRolesByUser($model->id):[];
if ($model->Login || count($roles)) {
?>
<h4>Доступ к инвентаризации</h4>
<?php
if ($model->Login && \Yii::$app->params['useRBAC'] || count($roles)) {
	...
	if ($model->Login && \Yii::$app->params['localAuth']) {
		echo \yii\helpers\Html::a('Сбросить пароль',['site/password-set','id'=>$model->id]);
	}
}?>
