...
class Schedules extends \app\models\base\ArmsModel
{
	...
	public $linksSchema=[
		...
		//записи и перекрытия - часть самого расписания, они удаляются вместе с ним
		//(beforeDelete), поэтому не должны блокировать его удаление
		'entries_ids' =>			[SchedulesEntries::class,'schedule_id','deletable'=>true],
    ...
    public function rules()
    {
        return [
			...
			//skipOnEmpty=false: проверка нужна именно на ПУСТОМ имени (inline-валидаторы
			//по умолчанию пустые значения пропускают)
			[['name'],'validateIndividual','skipOnEmpty'=>false],
        ];
    }

	/**
	 * Инвариант индивидуального (безымянного) расписания: у него не может быть
	 * ни дочерних расписаний, ни второго владельца - выбрать его где-либо еще
	 * невозможно (скрыто из fetchNames), поэтому такие ссылки стали бы висячими.
	 * Проверяется при попытке убрать имя у уже существующего расписания
	 * (у нового записи ни детей, ни владельцев быть не может).
	 * @param string $attribute
	 * @return void
	 */
	public function validateIndividual($attribute)
	{
		if ($this->isNewRecord || !$this->isPrivate) return;

		//считаем по свежей копии: связи текущего объекта могли закэшироваться пустыми
		//еще при его создании - до того, как появились владельцы и дочерние расписания
		$actual=static::findOne($this->id);
		if (!is_object($actual)) return;

		if (count($actual->childrenNonOverrides)) $this->addError($attribute,
			'Убрать имя нельзя: на это расписание ссылаются дочерние расписания '
			.'(индивидуальное расписание нельзя выбрать в качестве исходного)'
		);

		if (count($actual->usedBy)>1) $this->addError($attribute,
			'Убрать имя нельзя: расписание используется несколькими объектами '
			.'(индивидуальное расписание принадлежит только одному)'
		);
	}
	...
	public function attributeData()
	{
		return [
			...
			'name' => [
				...
				'Наименование',
				'hint' => 'Название расписания/временного доступа.<br>'
					.'Если название не указать, расписание станет <b>индивидуальным</b>: '
					.'оно не попадет в общий список расписаний, его нельзя будет выбрать '
					.'для другого объекта, а когда его единственный владелец '
					.'(сервис, регламентная работа) будет удален или переключен на другое '
					.'расписание - оно удалится вместе с ним.',
				'placeholder' => 'Без названия: индивидуальное расписание этого объекта',
			],
			'override_id' => [
				'Перекрывает',
	...
	/**
	 * Каскадно удаляем все, что является частью самого расписания: записи (дни/даты/периоды),
	 * перекрытия и списки доступа. FK в БД выпилены (M251221163631ClearFk), поэтому
	 * каскад держится только на этом методе - без него строки сиротеют.
	 * @return bool
	 */
	public function beforeDelete()
	{
		...
		if (count($this->acls)) {
			foreach ($this->acls as $acl)
				if (!$acl->delete()) return $this->cancelDelete();
		}

		foreach ($this->overrides as $override)
			if (!$override->delete()) return $this->cancelDelete();

		foreach ($this->entries as $entry)
			if (!$entry->delete()) return $this->cancelDelete();

		return parent::beforeDelete();
	}

	/**
	 * Снимает метку удаления (см. beforeDelete) и запрещает удаление.
	 * @return bool всегда false
	 */
	private function cancelDelete(): bool
	{
		unset(static::$deleting[$this->id]);
		return false;
	}
	...
	/**
	 * Список расписаний для выбора (select2 всех FK на расписание).
	 * Не предлагаем к выбору то, что не является самостоятельным расписанием:
	 * перекрытия (override_id), расписания временных доступов (свой раздел UI)
	 * и индивидуальные расписания - безымянные, принадлежащие одному объекту
	 * (issue #139: выбрать их где-то еще нельзя по определению).
	 */
	 ...
	 * - предзаполнение формы создания расписания со страницы владельца
	 *   ({@see \app\modules\schedules\controllers\SchedulesController::actionCreate()});
	 * - вычисляемое имя индивидуального расписания ({@see SchedulesModelCalcFieldsTrait::getGeneratedName()});
	 * - распознавание шаблонных (не авторских) имен при зачистке
	 *   ({@see \app\console\commands\SchedulesController::actionCleanupNames()}).
	 *
	 * @param \app\models\base\ArmsModel|null $owner объект-владелец (сервис/регламентная работа)
	 * @param string $mode ключ применения расписания: providing|support|job
	 * @return string
	 */
	public static function generateName($owner, string $mode): string
	{
		if (!is_object($owner) || !strlen((string)$owner->name)) return '';
		$name=StringHelper::mb_lcfirst($owner->name);
		switch ($mode) {
			case 'providing':	return static::$title.' работы '.$name;
			case 'support':		return static::$title.' поддержки '.$name;
			case 'job':			return static::$title.' '.$name;
		}
		return '';
	}

	/**
	 * @var bool[] id расписаний, которые удаляются прямо сейчас.
	 * Страховка от рекурсии gc <-> beforeDelete: удаление расписания каскадом
	 * удаляет своих ACL, каждый из которых через ScheduleOwnerBehavior зовет gc()
	 * того же расписания.
	 */
	private static $deleting=[];

	/**
	 * Сборщик мусора индивидуальных расписаний (issue #139).
	 *
	 * Вызывается владельцем расписания (через {@see \app\modules\schedules\components\ScheduleOwnerBehavior})
	 * по СТАРОМУ значению ссылки - при удалении владельца или переключении его на
	 * другое расписание. Удаляет расписание, только если оно осталось без единого
	 * владельца И не может быть переиспользовано.
	 *
	 * @param int|null $id расписание-кандидат (обычно старое значение FK)
	 * @param bool $dropNamed удалять даже именованное: владелец единолично владеет
	 *        расписанием (временный доступ - расписание создается вместе с ACL и
	 *        в общий пул не попадает, см. fetchNames)
	 * @return bool было ли расписание удалено
	 */
	public static function gc($id, bool $dropNamed=false): bool
	{
		if (!$id || isset(static::$deleting[$id])) return false;

		/** @var Schedules $schedule */
		$schedule=static::findOne($id);
		if (!is_object($schedule)) return false;

		//именованное расписание остается в общем пуле - его можно выбрать снова
		if (!$schedule->isPrivate && !$dropNamed) return false;
		//еще есть владельцы - расписание живое
		if (count($schedule->usedBy)) return false;
		//инвариант нарушен (ссылки из дочерних расписаний/вложения) - в мусор не отправляем,
		//такое расписание видно в списке по тумблеру "Индивидуальные" и удаляется руками
		if (count($schedule->childrenNonOverrides)) return false;
		if (count($schedule->attaches)) return false;

		try {
			return (bool)$schedule->delete();
		} catch (\Throwable $e) {
			//сборка мусора не должна ломать удаление/сохранение владельца:
			//расписание останется сиротой и будет подобрано командой schedules/gc
			\Yii::warning('Не удалось удалить индивидуальное расписание #'.$id.': '.$e->getMessage(),__METHOD__);
			return false;
		}
	}
