...
- **Колонка БД:** `schedules.compiled_json TEXT NULL` (размер по умолчанию СУБД). JSON сериализуется без форматирования.
- **Временная зона:** все `_tsm` в скомпилированном JSON хранятся в **UTC-минутах от Unix epoch**. Поле `tz` / `tz_shift_tsm` используется только при парсинге исходных строковых дат расписания и при локализованном выводе. Рантайм считает все сравнения в UTC.
- **Горизонт компиляции:** компилируются все имеющиеся данные без ограничения по датам (периоды/даты/overrides включаются целиком). Ограниченный горизонт (например, ±1 год) можно будет добавить позже, если JSON вырастет.
...
#### Требования к реализации валидаций

Все валидации реализуются в соответствующих моделях (`Schedules` / `SchedulesEntries`) через `rules()` или `beforeValidate()`:

| Что валидируем | Где | Правило | Сообщение |
| -------------- | --- | ------- | --------- |
| Пересечение периодов | `SchedulesEntries` (where тип=period) | При сохранении периода `[start, end)` внутри родительского `schedule_id` не должно быть других периодов с ненулевым пересечением: `other.start < new.end && new.start < other.end`. | «Период пересекается с уже существующим периодом расписания» |
| Пересечение overrides | `Schedules` (where `override_id IS NOT NULL`) | При сохранении override-расписания с `override_id = X` и датами `[start_date, end_date)` не должно быть других overrides на `X` с ненулевым пересечением дат. | «Перекрытие пересекается с существующим перекрытием расписания» |
| Уникальность weekday-entries | `SchedulesEntries` | В одном `schedule_id` не должно быть двух entries на один день недели (1..7). | «На день недели уже есть запись» |
| Уникальность date-entries | `SchedulesEntries` | В одном `schedule_id` не должно быть двух entries на одну дату. | «На эту дату уже есть запись» |

При провале любой из валидаций запись не сохраняется и перекомпиляция `compiled_json` не запускается.
...
1. **Шаг 1: Вычитание** — вычитаем override из всех интервалов (освобождаем место для override) через `intervalsSubtract()`
...
**Алгоритм:** период `[start, end)` пересекается с днём `[dayStart, dayEnd)` тогда и только тогда, когда выполнены **оба** условия:

- `period.end_tsm > dayStart` — период заканчивается позже начала дня (правая граница периода исключена — если `end == dayStart`, период считается уже закончившимся до дня)
- `period.start_tsm < dayEnd` — период начинается раньше конца дня

Это общая формула пересечения полуоткрытых интервалов `[A,B) ∩ [C,D) ≠ ∅ ⟺ A < D && C < B`. Следствие: пустой период (`start_tsm == end_tsm`) ни с каким днём не пересекается.
...
function getDatePeriods(date_tsm) {
    ...
    for (const period of periods) {
        // Период пересекается с днём, если:
...
| 5 | Edge | Период нулевой длины (одна минута/timestamp): start_tsm=28414800, end_tsm=28414800, date_tsm=28414800 | Период НЕ включён | Пустой период `[start, end)` с `start == end` ни с каким днём не пересекается: `end_tsm > dayStart` → `28414800 > 28414800` = false |
...
**Описание:**
1. **negative** (is_work=false): вычитаются из интервалов через `intervalsSubtract(intervals, neg)`
2. **positive** (is_work=true): добавляются через `intervalsAdd(intervals, pos)` БЕЗ merge соседних интервалов — см. подробнее в описании intervalsAdd
...
| 3 | Happy Path | pos=28416600, target=main, нет override/periods, есть weekday 3 (среда) с intervals=[[480, 1020, {}]] | weekday вернуть из nextWeekDayEntry | День недели с рабочим графиком или default |
...
| 11 | Integration | Вызов nextOverride, nextWorkDateEntry, findPeriod, nextWeekDayEntry, filterBefore | Корректный результат со всеми интеграциями | Полная цепь всех вызовов |
...
- **Версионирование:** нужно ли хранить историю версий скомпилированных расписаний — **решение: нет**.
- **Горизонт компиляции:** **решение: компилируется весь набор данных без ограничений по датам** (см. раздел «Зафиксированные решения»). При разрастании JSON вернёмся к идее ±1 года отдельной задачей.
