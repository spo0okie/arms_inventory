...
├── components/
│   └── ScheduleOwnerBehavior.php       — поведение владельца расписания (инвариант + сборка мусора индивидуальных)
├── helpers/
...
| [`ScheduleOwnerBehavior`](components/ScheduleOwnerBehavior.php) | Поведение владельца расписания: инвариант «одно индивидуальное расписание — один владелец» и сборка мусора при отвязывании |
| [`SchedulesController` (console)](../../console/commands/SchedulesController.php) | Обслуживание: зачистка шаблонных имён и сборка накопившихся сирот |
...
## Индивидуальные (безымянные) расписания

**Индивидуальное расписание — расписание с пустым `name`.** Оно принадлежит ровно
одному объекту-владельцу (сервису или регламентной работе), в общем пуле расписаний
не участвует и живёт ровно столько, сколько живёт его владелец (issue #139).

Мотивация: расписаний много, и большая их часть — одноразовые, заведённые «для вот
этого сервиса». В общем списке и в выпадающих списках выбора они только мешают,
а после удаления владельца превращаются в мусор.

### Инвариант

Индивидуальное расписание:

- **не предлагается к выбору нигде** — [`Schedules::fetchNames()`](models/Schedules.php)
  отсекает безымянные, а через неё построены select2 всех ссылок на расписание
  (`providing_schedule_id`, `support_schedule_id`, `schedules_id`, `parent_id`);
- **не показывается в общем списке** `/schedules` — [`SchedulesSearch`](models/SchedulesSearch.php)
  добавляет `name <> ''`; служебный тумблер «Индивидуальные» (`?showIndividual=1`)
  показывает их все одним списком — это режим «посмотреть, что накопилось»,
  а не обычный сценарий (обычно такое расписание открывают со страницы владельца);
- **имеет не более одного владельца** и **не имеет дочерних расписаний** — иначе
  ссылки на него стали бы висячими: выбрать его заново невозможно;
- **удаляется, потеряв владельца** — сразу, в момент удаления владельца или
  переключения его на другое расписание.

Перекрытия (`override_id`) и записи (`schedules_entries`) — часть самого расписания,
они инвариант не нарушают и удаляются вместе с ним.

### Где это удерживается (все пути записи)
...
Сторож [`ScheduleOwnersGuardTest`](../../tests/unit/modules/schedules/ScheduleOwnersGuardTest.php)
валит CI, если появилась пятая ссылка без `ScheduleOwnerBehavior`: без него
индивидуальное расписание молча осиротеет.

### Имя в интерфейсе

Своего имени у индивидуального расписания нет, поэтому в заголовках и служебных
списках оно называется **вычисленным** именем — по владельцу:

```php
Schedules::generateName($service,'providing');  // 'Расписание работы почты'
$schedule->generatedName;                       // то же, но по фактическому владельцу
$schedule->displayName;                         // name, а у безымянного - generatedName
```

Формула одна на всех потребителей (`generateName`) — вычисляемое имя, распознавание
шаблонных имён при зачистке и рендер списков не должны разойтись.

В карточке расписания у индивидуального выводится пояснение режима и ссылка
**«Сделать общим»** — она ведёт в форму редактирования с уже подставленным
вычисленным именем: обратный ход к общему пулу стоит одного нажатия.

### ACL-расписания (временные доступы) — особый владелец

Расписание временного доступа создаётся вместе с доступом
([`AclsController::actionCreate()`](../../controllers/AclsController.php)),
делится между всеми ACL этого доступа и в общий пул не попадает (та же `fetchNames`
отсекает расписания с ACL). Поэтому `Acls` подключает behavior с `exclusive=true`:

- несколько ACL одного доступа могут ссылаться на одно расписание (это не «чужое»);
- с последним ACL расписание удаляется **даже если у него есть имя** — иначе оно
  всплывает в общем списке расписаний как мусор (так было до issue #139).

> **Готча.** `exclusive` означает «владелец владеет расписанием безраздельно».
> Если ссылку `acls.schedules_id` проставить на общее расписание из пула напрямую
> (REST/консоль — в UI такого выбора нет), то удаление последнего ACL унесёт и его.

### Обслуживание накопившегося

```bash
# снять ШАБЛОННЫЕ имена с одноразовых расписаний (сделать их индивидуальными)
php yii schedules/cleanup-names            # отчёт по группам, ничего не меняет
php yii schedules/cleanup-names --dryRun=0 # применить

# подобрать индивидуальные расписания, оставшиеся без владельцев
php yii schedules/gc
php yii schedules/gc --dryRun=0
```

`cleanup-names` снимает имя, **только если оно посимвольно совпадает с шаблоном**
`Schedules::generateName()` для текущего владельца. Такое имя когда-то предзаполнила
форма создания расписания со страницы владельца — авторского смысла в нём нет, оно
восстанавливается формулой, поэтому операция ничего не теряет: расписание продолжит
называться так же. Имена, написанные человеком, не трогаются; расписания, у которых
владельца переименовали (имя разошлось с шаблоном), попадают в отчёт отдельной
группой — решать глазами.

> Форма создания расписания со страницы владельца имя **не предзаполняет**: новое
> расписание по умолчанию индивидуальное. Чтобы сделать его общим, имя вписывают руками.
