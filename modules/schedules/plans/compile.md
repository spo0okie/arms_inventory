...
**ВАЖНО:** при наложении периодов интервалы НЕ склеиваются, даже если соседние имеют одинаковую meta! Границы каждого интервала сохраняются как отдельный элемент.
...
2. Для каждого периода:
   - если is_work=true: добавить intervals к рабочим **БЕЗ merge**
   - если is_work=false: вычесть intervals из рабочих
3. **ПРИМЕЧАНИЕ:** No merging after overlays! Соседние интервалы остаются отдельными.
...
| 2 | Happy Path | intervalsAdd([[480, 1020, {}]], [[300, 500, {}]]) | [[300, 500, {}], [480, 1020, {}]] | Override не пересекается: добавляем как новый элемент |
...
| ------- | -------- | ------------- |
| [`intervalsContains(intervals, tsm)`](modules/schedules/plans/compile.md:853) | Найти интервал из набора, который содержит tsm (null, если не содержит). Проверка: minutesFromDay >= start && minutesFromDay < end | isWorkTime, getMeta |
| [`intervalsIntersect(a, b)`](modules/schedules/plans/compile.md) | Найти пересечение двух интервалов [start, end] | internalUse |
| [`intervalsMerge(intervals)`](modules/schedules/plans/compile.md) | Объединить пересекающиеся интервалы в один (только для компиляции БД) | validation |
| [`intervalsSubtract(intervals, remove)`](modules/schedules/plans/compile.md:879) | Вычесть из набора интервалов intervals один интервал remove. Может вернуть 0, 1 или 2 интервала в месте вычитания | applyPeriodsToDay, intervalsAdd |
| [`intervalsAdd(intervals, override)`](modules/schedules/plans/compile.md:428) | Добавить интервал override с приоритетом его meta. **КРИТИЧНО: NO MERGE** — интервалы НИКОГДА не объединяются, даже если соседние имеют одинаковую meta! | applyPeriodsToDay |
| [`filterBefore(entry, tsm)`](modules/schedules/plans/compile.md:537) | Отфильтровать интервалы записи, оставив только те, которые ещё не завершились к моменту tsm. Если запись не на текущий день — возвращается оригинал. Интервал завершён если end <= minutesFromDay | nextWorkDateEntry |
