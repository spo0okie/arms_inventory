...
/**
     ...
     *
     * Дублями считаются живые записи, у которых поле name встречается более одного
     * раза внутри одного окружения ({@see Comps::dupeIds()}): клоны в песочницах
     * намеренно носят имя продуктива и дублями не считаются, архивные записи
	 ...
	 * - выполняет агрегирующий SQL-запрос по `comps.name`+`comps.sandbox_id` среди
	 *   неархивных записей и ищет те, у которых одинаковый `name` встречается более
	 *   одного раза внутри одного окружения (клоны в песочницах дублями не считаются);
	 ...
	 *   первую, а у обеих снимается `archived` (архивные в дубли не попадают) —
	 *   всё через `silentSave()`, чтобы не проходить бизнес-валидации формы и
	...
	public function testDupes(): array
	{
		...
		try {
			...
			// Принудительно выравниваем name и окружение и снимаем архивность,
			// чтобы получить пару дублей в БД (архивные в дубли не попадают).
			$first->archived = 0;
			$first->silentSave();
