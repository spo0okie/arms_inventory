...
├── web/                 # Публичная директория (DocumentRoot веб-сервера)
...
Корень сайта (DocumentRoot) - подпапка [`web/`](web), а не корень проекта: код, конфиги
и `runtime` остаются вне зоны досягаемости веб-сервера, а адреса приложения не содержат
лишних токенов (`/techs/index`, `/api/users`).

Первый релиз публиковался иначе - DocumentRoot указывал на корень проекта, а
[`.htaccess`](.htaccess) переписывал запросы в `web/`, из-за чего во всех адресах оседал
токен `/web`. Такие адреса до сих пор в ходу у внешних интеграций, поэтому поддержаны
явно: [`web/.htaccess`](web/.htaccess) срезает префикс на уровне веб-сервера (чтобы по
старому адресу отдавалась и статика), а [`app\components\Request`](components/Request.php) -
из пути запроса. Совместимость прозрачная, без редиректа, и выключается настройкой
`request.legacyPathPrefix=null`. Подробности - [docs/help/admin/install.md](docs/help/admin/install.md).
...
- Провайдеры: [`components/integrations/providers/`](components/integrations/providers):
  ...
  - `AdUserProvider` — учётка ActiveDirectory сотрудника (поверх
    ldap-компонента): панель-справка + именные действия (L2+: личные
    креды исполнителя) — сброс пароля, создание учётки (формы
    `AdCreateAccountForm`: логин транслитом «фамилия.и», дерево OU,
    группы) и восстановление после увольнения (`AdRestoreAccountForm`:
    включение + пароль + переезд по зеркалу пути увольнения). Пароли
    доставляются через SMS-провайдера и не показываются исполнителю;
    кнопки — внутри панели, по живому состоянию учётки в AD;
