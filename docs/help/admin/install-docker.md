...
> Настройки приложения при docker-инсталляции лежат в папке **config** рядом с docker-compose.yml: файл **config/params.php** монтируется внутрь контейнера как params-local.php (см. [настройку](setup.md)), **config/ldap.php** - как конфиг LDAP.
