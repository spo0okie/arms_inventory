...
[отпечатках софта](comps/raw_soft.md) операционных систем.
...
- **основные элементы** — совпадение хотя бы одного с строкой отпечатка
  означает «продукт установлен на этой ОС» (обнаружение);
...
служебных списка управляют раскладкой блока «Софт» на карточке ОС:
...
[типы лицензий](lic-groups.md), к которым уже привязываются закупки
и ключи. Схема учёта — в гайде [Лицензии](../guides/licenses.md).
...
- **шапка** — разработчик и наименование, описание, ссылки,
  изображения (скриншоты, сканы коробок);
- вкладки:
  - **Обнаружения** — ОС, на которых продукт распознан; колонка
    «В паспорте» показывает, закреплён ли он там же вручную;
  - **Совместимые лицензии** — типы лицензий, покрывающие продукт,
    со счётчиком активных лицензий;
...
- из карточки ОС — кнопка «Создать продукт из этого элемента» у
...
- Наименование вносится **без** имени разработчика — если название
  вендора продублировано в наименовании, при сохранении оно будет
  автоматически отрезано;
- элементы пакета — regexp-выражения: спецсимволы (`( ) . +` и т.п.)
  нужно экранировать;
...
Удалить продукт можно кнопкой-корзиной в окне просмотра: привязки
к ОС (обнаружения и паспорта) при этом снимаются автоматически.
