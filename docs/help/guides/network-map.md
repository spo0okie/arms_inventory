...
- **синий пунктир «MAC»** и раздел «Как соединены — по таблицам MAC» — для
  ...
  звено — между портами может стоять неуправляемая коробка, поэтому запись
  предлагается только руками, в форме порта. Взаимно-однозначные пары
  (порт А видит только Б, а порт Б — только А) — почти наверняка прямой
  линк, они и рисуются на схеме. Самый надёжный способ получить полную
  карту — включить LLDP на коммутаторах;
