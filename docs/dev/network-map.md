...
- **Когда LLDP молчит** (у смартов выключен из коробки): шапка считает
  ...
  коробка), поэтому в запись не предлагается; но взаимно-однозначная пара
  (порт А видит только Б, порт Б — только А) рисуется на схеме синим
  пунктиром «MAC: порт—порт» как вероятный прямой линк, запись — руками.
  Полный список направлений — свёрнутой таблицей под находками.
