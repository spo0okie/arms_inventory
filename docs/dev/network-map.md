...
- Адреса сопоставляются с объектами и среди оборудования, и **среди ОС**
  ...
  отдельное поле `found_os`: рисуется строкой «ОС без оборудования: …»
  (ссылка на карточку), но без кнопки привязки — порт соединяется с железом,
  привязывать не к чему; починка — указать АРМ в карточке ОС. На карте такой
