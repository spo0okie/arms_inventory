...
приходят все разом — по ним серийники сверяются с карточками каждого члена.
Панель опроса показывает блок «коммутатор о себе» со сверкой
(`MacSearchProvider::identityReport()`): sysName против hostname (по
короткому имени), базовый MAC против поля MAC (чужая карточка с этим
адресом — коллизия и называется по имени), серийники юнитов против `sn`
членов стека (обратная сторона тоже: серийник в карточке, которого
устройство не показало), модель — нестрогим вхождением; sysDescr и
sysLocation — справочно. Расхождения подсвечены.
