...
Строки внутри ячейки — те же, что в строке таблицы (порт, VLAN, пояснение,
соединение): в вертикальном письме они ложатся РЯДОМ, и подпись выходит
вдвое короче, чем одной лентой. Поэтому ширину колонки диктуют сами данные —
`PortsLayoutWidget::columnWidth()` считает её по девятому децилю числа строк
(по максимуму нельзя: один порт с цепочкой растянул бы весь корпус, розетки
разъехались бы на пол-экрана; потолок — `MAX_LINES`). Розетка при этом не
растягивается на всю колонку (`max-width`), иначе корпус перестаёт быть
похож на корпус. Что не влезло — в подсказке ячейки и целиком в таблице.

Готчи вида: действие идёт сразу за именем порта (в конце колонки его может
не быть видно); пометки и значки поворачиваются вместе с подписью, а
`.badge` в колонке рисуется без заливки — повёрнутая плашка перекрывает
соседние строки; «ёлочка» строки схлопывает (`br { display: none }`), и
