...
веер параллелит цели, а не только таблицы. Весь опрос ограничен
`scan.deadline` сервиса (по умолчанию 240 с, 0 — ждать всех): по дедлайну
отдаётся собранное, неуспевшие цели уходят в `errors` поимённо («не
уложился в срок опроса») и дорабатывают в фоне; время каждой цели пишется
в журнал сервиса строкой «полный опрос занял N с» — по ней видно, кто
тормозит площадку. Терпение сверки карты на стороне ARMS — 8 попыток по
~40 с (перекрывает дедлайн), настраивается `mapMaxAttempts` в конфиге
интеграции.
