...
interface AttributeTypeInterface extends GeneratorInterface
{
	 ...
	 * (issue #194). Для типов-«списков» (отпечатки софта/железа и т.п.)
	 * вместо генерик-подачи «старое → новое» показываются только изменения.
	 * Контракт результата:
	 * - null — у типа нет собственного diff-представления (потребитель
	 *   падает на генерик: множества/«старое → новое»), в т.ч. если
	 *   значения не разобрались;
	 * - ['added'=>[], 'removed'=>[], 'changed'=>[]] — списки готовых
	 *   HTML-элементов: добавленные, выбывшие и обновлённые (пары
	 *   выбывший+добавленный, опознанные как одна сущность) элементы.
	 * Дефолтная реализация (null) — в BaseType.
	 */
