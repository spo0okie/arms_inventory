...
## Порядок изменения стандартов

Репозиторий сопровождается одним разработчиком, поэтому стандарты меняются
без внешних согласований: если правило перестало быть целесообразным —
оно меняется прямо в этом файле (или в профильном документе) тем же коммитом,
что и код, который ему следует. Единственное требование — изменение стандарта
фиксируется в документации, а не остаётся устной договорённостью.
...
**Порядок записей в `attributeData()`:** ключи держать отсортированными по
алфавиту — у больших моделей этот блок занимает сотни строк, и без сортировки
атрибут визуально не найти. Исключение: alias-запись
(`'accessTypes'=>['alias'=>'access_types_ids']`) ставится вплотную к своему
базовому атрибуту. Новые атрибуты добавлять в отсортированную позицию, а не в
конец; попутная пересортировка не отсортированного легаси-блока приветствуется
отдельным коммитом (чтобы не смешивать с содержательными правками).

Обратная карта — «от UI-элемента к источникам данных/документации»
(двуслойная документация + классы типов) и шаблоны описаний атрибутов,
типов и MD-страниц: [`ui-sources.md`](ui-sources.md).
...
Документация приложения — **встроенная**, ведётся в репозитории (docs-as-code)
и меняется в тех же коммитах, что и код:

- **Слой 1 (код):** `hint` в `attributeData()`, `inputHint()/searchHint()`
  типов, `modelDescription()` моделей. Показывается тултипами
  (сборщик — `components/AttributeTooltip`, спецификация [`ui-sources.md`](ui-sources.md) §0.1).
- **Слой 2 (MD):** [`docs/help/`](docs/help/README.md) — подробные страницы
  сущностей/атрибутов/типов, сценарии, администрирование. Конвенция и шаблоны:
  [docs/help/README.md](docs/help/README.md) и [`ui-sources.md`](ui-sources.md).

**Правило актуальности: изменил семантику поля, типа или сущности — обнови
hint/modelDescription/MD-страницу в том же коммите.**

Сторожа (tests/unit/help/): HelpCoverageTest — новый атрибут/модель без
описания валит CI (baseline существующих пробелов может только уменьшаться);
HelpOrphanTest — MD-страницы без модели/атрибута/типа; HelpLinksTest —
битые относительные ссылки.

Внешняя wiki для документации приложения **не используется**: wiki-механизмы
(`WikiController`, `WikiPageWidget`, `params['wikiUrl']`) существуют только
для пользовательского контента с wiki-разметкой.
