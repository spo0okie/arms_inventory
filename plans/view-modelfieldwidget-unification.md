...
Мелочь-хвост — ✅ СДЕЛАНО (2026-07-15): неиспользуемые `use …UrlListWidget` в
`tech-models/view.php`, `lic-types/card.php`, `soft/card.php` вычищены.

`user-groups/card.php` — ПУНКТ СНЯТ: user_groups выпилены целиком
(миграция m260711 drop_user_groups), вьюх больше нет.
...
### Группа 4. Дообъявить `typeClass` простым `links`/text-атрибутам — ✅ СДЕЛАНО (2026-07-15)

`lic-types.links` — в существующую запись attributeData добавлен
`typeClass=UrlsType`. `soft.links` — записи в attributeData не было вовсе,
заведена (label «Ссылки» + hint + typeClass). Unit 656 + help-сторожа зелёные
(HelpCoverage новый hint не сломал).
