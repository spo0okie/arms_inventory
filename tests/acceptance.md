...
controllers/*Controller.php
  ...
  -> сценарии (GET/POST + ожидаемый HTTP-код + optional assert)
  -> проверка доступности маршрута
  -> optional post-assert (проверка результата действия в БД/модели)
```
...
- `assert` (`callable`) — дополнительная проверка после HTTP-ассерта.
  ...
  Используется для подтверждения бизнес-результата action (например, что данные реально изменились в БД);
- `skip` (`bool`) — пропуск сценария;
- `reason` (`string`) — причина skip;

---

### Пример сценария с assert
...
return [[
    ...
    'assert' => static function () use ($userId, $newPassword) {
        ...
        Assert::assertTrue($user->validatePassword($newPassword));
    },
]];
```

`assert` выполняется только после успешной проверки `response`.
Если нужна только проверка доступности страницы/маршрута — `assert` не обязателен.
...
- в `tests/acceptance.md` описываем **контракт сценариев, HTTP-ожидания и post-assert проверки**;
...
- `assert`-проверка падает: HTTP успешный, но бизнес-результат не достигнут (например, запись не обновилась).
