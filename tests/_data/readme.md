...
| `10-dns-names.sql` | внешняя зона taburetka.ru с именами на белых адресах DMZ и шлюзов, алиасы узлов во внутренней зоне, имя на два адреса, имя без адресов (парковка) — карта зоны на карточке домена |
| `11-forwards.sql` | записи доступа с признаком «Проброс» (типы обычные: HTTPS, Ovpn): пробросы с белых адресов шлюзов на реверс-прокси и VPN-серверы: блоки «Доступен снаружи»/«Пробрасывается на» у ОС, IP и DNS-имён |
| `12-transit.sql` | транзитные межсервисные связи: два сервиса ходят в 1С через прокси, указатели «следующий хоп», колонка «Маршрут» и блок маршрута на карточке записи доступа |
| `13-segments-access.sql` | сегмент как ресурс ACL и субъект ACE, матрица межсегментного доступа (в т.ч. запись с несегментным субъектом, которую матрица игнорирует), сети разложены по сегментам |
