...
<table class="table table-sm small mt-1">
	<thead><tr>
		<th>Порт</th>
		<?php if ($hasPassport) { ?>
			<th qtip_ttip="Тип интерфейса (ifType): розетка на корпусе только у ethernet">Тип</th>
			<th qtip_ttip="admin / oper: включён ли администратором / есть ли линк">Состояние</th>
			<th>Скорость</th>
			<th qtip_ttip="Описание порта на самом коммутаторе (ifAlias)">Подпись</th>
			<th qtip_ttip="Агрегированный канал, в который собран порт">Группа</th>
			<th qtip_ttip="Настроенные VLAN; нетегированный жирным">VLAN</th>
		<?php } ?>
		<th qtip_ttip="Адреса из таблицы MAC и VLAN, в котором каждый замечен">Адреса</th>
