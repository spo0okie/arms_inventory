...
abstract class IntegrationProvider
{
	...
	/**
	 * @var bool результат последнего рендера панели годится в кэш.
	 * Провайдер сбрасывает флаг, когда отдал не результат, а состояние -
	 * «опрос ещё идёт», «сервис недоступен»: закэшированный спиннер висит
	 * до истечения ttl и не убирается ничем, а закэшированная ошибка
	 * десять минут врёт, что сервис лежит. Proxy читает флаг после рендера.
	 */
	...
	/**
	 * Наполнить ячейки колонки для набора моделей — ЕДИНСТВЕННОЕ место
	 * внешних вызовов списочного режима: один набор = один поход во
