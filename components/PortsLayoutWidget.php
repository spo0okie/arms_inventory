 ...
 * нижнего сверху вниз (начало тоже у розетки). Внутри колонки ячейки строки
 * таблицы идут друг за другом ВДОЛЬ неё, а вторая строка каждой (VLAN под
 * именем порта, находки под связью) разворачивается ВБОК - поэтому ширина
 * колонки равна двум строкам и считается по самим подписям
 * ({@see columnWidth()}). Розетки и подписи лежат в одной сетке, а не в двух
 * выровненных «на глаз»: только так подпись указывает на свой порт.
 *
...
class PortsLayoutWidget extends Widget
{
	...
	/**
	 * Запас ширины сверх самой подписи: колонка, подогнанная в притык, ставит
	 * описания соседних портов вплотную и они читаются как одно. Плюс треть -
	 * это пустое поле между ними
	 */
	const SPACING = 1.3;

	/** Минимальная ширина колонки в em (до запаса): номер порта уже не бывает */
	const MIN_WIDTH = 2.6;
	 ...
	 * а по девятому децилю: один порт с цепочкой из трёх устройств иначе
	 * растянул бы весь корпус, и розетки разъехались бы на пол-экрана.
	 * Строка подписи - примерно 0.95em, плюс поля и запас {@see SPACING}.
	 */
	protected function columnWidth(): float
	{
		$width = static::MIN_WIDTH;
		if (count($this->lines)) {
			$lines = $this->lines;
			sort($lines);
			$typical = $lines[(int)floor((count($lines) - 1) * 0.9)];
			$width = max($width, min($typical, static::MAX_LINES) * 0.95 + 0.5);
		}
		return round($width * static::SPACING, 2);
	}
	...
	/** Ряд розеток: подпись - номер порта, раскраска - вердикт опроса */
	...
	/**
	 * Ряд подписей: строка таблицы, поставленная на попа над своим портом
	 * (`up`) или под ним (`down`).
	 */
	protected function renderCells(array $row, array $byName, PortsRowRenderer $renderer, string $side): string
	{
		...
		foreach ($row as $name) {
			...
			//порта нет на корпусе либо о нём нет строки: пустая колонка, чтобы
			//соседи не сдвинулись
			$content = '';
			if (!is_null($port)) {
				[$content, $lines] = $this->cellContent($port, $renderer);
				$this->lines[] = $lines;
			}
			...
			$html .= Html::tag('div',
				strlen($content) ? Html::tag('span', $content, ['class' => 'ports-layout-text']) : '',
				['class' => 'ports-layout-cell '.$side, 'data-port' => $name]);
		}
		return $html;
	}

	/**
	 * Подпись одного порта: ячейки строки таблицы в том же порядке, каждая -
	 * отдельным куском.
	 *
	 * Куски идут друг за другом ВДОЛЬ колонки, а своя вторая строка ячейки
	 * (VLAN под именем порта, описание под пояснением, находки под связью)
	 * разворачивается ВБОК - ровно как в таблице, только повёрнуто. Отсюда и
	 * ширина колонки: два ряда строк, а не сумма всех.
	 *
	 * Класса `port-scan-accept` на кусках нет намеренно: «принять однозначные
	 * разом» собирает кнопки по таблице, и вторая их копия в раскладке
	 * применила бы каждую находку дважды.
	 *
	 * @return array [html подписи, сколько строк вышло в самом «толстом» куске]
	 */
	protected function cellContent(array $port, PortsRowRenderer $renderer): array
	{
		$parts = $renderer->parts($port);
		$connection = $parts['connection'];
		//порядок - как колонки таблицы, слева направо
		$pieces = array_filter([
			$parts['port'],
			$parts['aggregate'],
			$parts['comment'],
		...
		foreach ($pieces as $piece) {
			...
			//таблицы: разделитель тут лишний, куски и так читаются по отдельности
			$html .= Html::tag('span', $piece, ['class' => 'ports-layout-part']);
		}
		return [$html, $lines];
	}
}
