...
class OrgStruct extends ArmsModel
{
	...
	public static function modelDescription(): string
	{
		return 'Дерево подразделений организаций; синхронизируется из кадровой системы '
			.'и строго соответствует кадровому делению. '
			.'Используется для привязки сотрудников и оборудования к подразделениям. '
			.'Если кадровая структура не отражает реальное функциональное распределение — '
			.'используется своё кастомное деление через справочник «Подразделения».';
	}
    ...
    public function attributeData()
    {
        return [
			...
			'hr_id' => [
				...
				'hint'=>'Идентификатор подразделения в кадровой (HR) системе',
			],
			'org_id' => [
				'Организация',
				'hint'=>'Контрагент (организация), которой принадлежит подразделение',
			],
            'parent_hr_id' => [
            	'Родительское подразделение',
				'hint'=>'Идентификатор вышестоящего подразделения в кадровой системе',
			],
			'parent_id' => [
				'Родитель',
				'hint'=>'Вышестоящее подразделение (внутренняя ссылка, строится по данным кадровой системы)',
			],
            'name' => ['Наименование подразделения','hint'=>'Название подразделения'],
        ];
    }
