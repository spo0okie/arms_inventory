...
DELIMITER ;
    ...
    public function attributeData()
    {
        return ArrayHelper::recursiveOverride(parent::attributeData(),[
	        ...
	        'parent' => [
	        	'Родитель',
				'hint' => 'Помещение внутри которого находится это.<br>'
					.'Если ничего не выбрать — создастся новый филиал (корневое помещение)',
				'placeholder'=>'Выберите родительское помещение',
				'typeClass'=>\app\types\LinkType::class,
			],
            'name' => [
            	'Полное имя',
				'hint' => 'Понятное название помещения без сокращений.<br>'
					.'Отображается при наведении мышки на короткое имя в карте помещений',
				'typeClass'=>\app\types\StringType::class,
			],
			'short' => [
				'Короткое имя',
				'hint' => 'Сокращенное название помещения для вывода в узких местах.<br>'
					.'Используется чаще полного: в выпадающих списках и карте помещений',
				'is_inheritable'=>true,
				'typeClass'=>\app\types\StringType::class,
			],
            'addr' => [
            	'Адрес',
				'hint' => 'Если не указан, то наследуется адрес родительского помещения.<br>'
					.'У корневого помещения (филиала) нужно указывать обязательно.<br>'
					.'Если у дочернего помещения адрес отличается — он выведется строчкой над помещением в карте',
				'is_inheritable'=>true,
