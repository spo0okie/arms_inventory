...
class Contracts extends ArmsModel
{
	...
	public function attributeData()
	{
		return [
			...
			'comment' => [
				'Комментарий',
				'typeClass'=>\app\types\TextType::class,
				'hint' => 'Для счетов желательно записывать историю и логистику закупки '
					.'в реверсивной хронологии (свежие записи сверху), '
					.'т.к. <b>первая строка комментария выводится в списке рядом со статусом</b> '
					.'(для быстрого уточнения текущего этапа закупки/логистики).<br>'
					.'Если пояснение статуса не нужно — оставьте первую строку пустой'
			],
	...
	/**
	 * Первый предшественник документа — корень цепочки замещений
	 * (самая первая версия; все заменяющие версии хранятся «звездой»
	 * вокруг него, актуальная определяется по дате)
	 * @return Contracts
	 */
	...
	public function beforeSave($insert)
	{
		if (parent::beforeSave($insert)) {
			...
			if ($this->is_successor) {
				//Если этот документ заменяет предыдущую версию договора, то пришиваем
				//его к КОРНЮ цепочки (самой первой версии): все версии хранятся «звездой»
				//вокруг корневого документа, а хронология замещения восстанавливается
				//сортировкой по датам (getSuccessorsChain) — так у цепочки одна точка
				//привязки, даже если пользователь выбрал предшественником промежуточную версию
