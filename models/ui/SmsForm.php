    ...
    public function rules()
    {
        return [
        	['phone', 'filter', 'filter' => function ($value) {
        		...
        		$value=trim($value);
        		//+7 -> 8
        		if (substr($value,0,2)==+7)
				/* убираем посторонние символы из номера*/
				$value=preg_replace('/[^0-9]/','',$value);
				return $value;
			}],
        	['phone','string', 'min'=>11, 'max'=>11,
				'tooShort'=>'Номер должен быть ровно 11 знаков без пробелов (начиная с 8)',
				'tooLong'=>'Номер должен быть ровно 11 знаков без пробелов (начиная с 8)',
			],
