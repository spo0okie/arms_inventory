...
class Techs extends ArmsModel
{
	...
	public function attributeData()
	{
		return ArrayHelper::recursiveOverride(parent::attributeData(),[
			...
			'comment' => [
				'Примечание',
				//TODO-REVIEW: "выводится в карте помещений" - утверждение из wiki, по коду не сверено
				'hint' => 'Краткое пояснение по этому оборудованию.<br>'
					.'Выводится при наведении на статус оборудования в карте помещений.<br>'
					.'У некоторых категорий оборудования поле заменяется (например, '
					.'для телефонов — на «номер телефона»)',
				'typeClass'=>\app\types\TextType::class,
			],
			'contracts_ids' => [
				'Связанные документы',
				//TODO-REVIEW: организационный регламент из wiki - подтвердить обязательность
				'hint' => 'Счета, накладные, фотографии серийных номеров и т.п.<br>'
					.'Сюда привязывается документ-основание приходования; '
					.'акты на ремонт также прикрепляются сюда',
			...
			'domain_id' => [
				'Домен',
				'hint'=>'Домен, к которому относится оборудование',
				'typeClass'=>\app\types\LinkType::class,
			],
			'lic_groups_ids' => [
				LicGroups::$titles,
				'hint'=>'Типы лицензий, привязанные к этому оборудованию/АРМ',
			],
			'lic_items_ids' => [
				LicItems::$titles,
				'hint'=>'Закупки лицензий, привязанные к этому оборудованию/АРМ',
			],
			'lic_keys_ids' => [
				LicKeys::$titles,
				'hint'=>'Лицензионные ключи, привязанные к этому оборудованию/АРМ',
			],
			'scans_id' => [
				'Сканы',
				'hint'=>'Сканы документов, прикреплённые к этому оборудованию',
			],
			...
			'head_id' => [
				...
				'hint' => 'Руководитель отдела сотрудника которому установлено.<br>'
			...
			//формат заполнения (диапазоны, примеры) подскажет RackUnitsType (inputHint)
			'installed_pos'=>[
				'Места установки',
				'hint'=>'Позиция этого устройства в корзине/шкафу:<br>'.
					'номера посадочных мест, которые занимает устройство<br>'.
					'(лицевая сторона, если позиции для обратной стороны отличаются)',
				'typeClass'=>\app\types\RackUnitsType::class,
			],
			'installed_pos_end'=>[
				'C обр. стороны',
				'hint'=>'Если обратная сторона устройства занимает другие позиции,<br>'.
					'то тут нужно указать номера посадочных мест для обратной стороны',
				'typeClass'=>\app\types\RackUnitsType::class,
			],
			'inv_num' => [
				'Бухг. номер',
			...
			'maintenance_reqs_ids'=>[
				...
				'hint'=>'Какие предъявлены требования по обслуживанию этого оборудования '
					.'<br>Вообще эти требования должны предъявлять сервисы, работающие на '
			...
			'places_id' => [
				'Помещение',
				//TODO-REVIEW: организационный регламент из wiki (2х-летней давности) - подтвердить
				'hint' => 'Помещение, куда установлено.<br>'
					.'Если оборудование надолго (больше пары дней) убыло в серверную/на склад — '
			...
			'state_id' => [
				...
				'hint' => 'Состояние, в котором в данный момент находится оборудование '
					.'(сломан, в ремонте, на хранении и т.п.).<br>'
					.'Список доступных состояний настраивается (меню Оборудование → Состояния)',
			...
			'uid' => [
				...
				'hint'=>Yii::$app->params['techs.uidHint']??'Какая-либо дополнительная маркировка нанесенная на оборудование<br>'
					.'(например сохранившаяся маркировка прежней организации, если она нанесена хорошо)',
			],
			'url' => [
				'Ссылки',
				//формат заполнения подскажет UrlsType (inputHint)
				'hint' => 'Ссылки на связанные страницы и ресурсы.<br>'
					.'Ссылка на http://&lt;IP устройства&gt; добавляется автоматически — '
					.'руками её вносить не нужно',
			],
		]);
	}
