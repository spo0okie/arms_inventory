...
class Services extends ArmsModel
{
    ...
    public function attributeData()
    {
        return array_merge(parent::attributeData(), [
			...
			'charge' => [
				'НДС',
				'hint' => 'Величина НДС, входящая в месячную стоимость',
			...
			'is_end_user' => [
				...
				'hint' => 'Заметят ли конечные пользователи, если это перестанет работать.<br>'
					.'Если потребитель — только ИТ-отдел или другие сервисы, галочку не ставим',
				'typeClass' => \app\types\BooleanType::class,
			],
			'is_service' => [
				'Тип объекта',
				'hint' => 'Сервис — техническая система, работает на серверах/оборудовании '
					.'(прокси, синхронизация с AD).<br>'
					.'Услуга — её кто-то оказывает: человек или контрагент '
					.'(предоставление интернета, обслуживание кондиционеров)',
				'typeClass' => \app\types\BooleanType::class,
			],
			'links' => [
				'Ссылки',
			...
			'name' => [
				...
				'hint' => 'Короткое уникальное название сервиса или услуги.'
					.'<br>В дереве сервисов и в навигационной цепочке начало имени, совпадающее с названием '
					.'или одним из <strong>"Альясов"</strong> основного сервиса, скрывается (для краткости). '
					.'Например <em>"Телефония - Мобильная связь"</em> внутри сервиса <em>"Телефония"</em> '
					.'будет показано как <em>"Мобильная связь"</em>.',
				'indexHint' => '{same}<br />'.
					'При поиске также ищет в полях '.
					'<strong>"Альясы"</strong> и '.
			...
			'search_text' => [
				...
				'hint' => 'Какие еще названия используются в отношении этого сервиса (по одному в строку). '
					...
					.'<br>Также в дереве сервисов и в навигационной цепочке начало имени дочернего сервиса, '
					.'совпадающее с названием или одним из альясов этого сервиса, скрывается (для краткости). '
					.'Например <em>"Телефония - Мобильная связь"</em> внутри сервиса с альясом <em>"Телефония"</em> '
					.'будет показано как <em>"Мобильная связь"</em>.',
				'typeClass' => \app\types\TextType::class,
			],
